package br.com.eduardo.calculaflex.watchers
...
class DecimalTextWatcher(editText: EditText, val totalDecimalNumber: Int = 2) : TextWatcher {
    ...
    private fun formatMoney(editable: Editable) {
        val editText = editTextWeakReference.get() ?: return // Evita memory leaks
        ...
        val parsed = when (cleanString) {
            null -> BigDecimal(0) // Valor maior que um decimal normal
            "" -> BigDecimal(0)
            "null" -> BigDecimal(0)
